...
tribunes.forEach(tribune => {
    tribune.addEventListener('click', () => {
        tribunes.forEach(tribune => {
            ...
            tribune.className = [...tribune.classList].filter((el, index) => index < 2).join(' ')
        })
        console.log(tribune.classList);
        const active = tribune.getAttribute('data-active')
        tribune.classList.add('tribune-active')
        tribune.classList.add(active)
        btn.disabled = false
